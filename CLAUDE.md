...
Packaging shims sit beside the file without changing its shape: `pyproject.toml` publishes it to PyPI as `duet-cli` (console script `duet = "duet:main"`, flat module `duet` via `py-modules`, `[yaml]` extra for PyYAML, zero runtime deps — bare `duet` on PyPI is Google's async library); `.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json` (marketplace name `volkan-duet`, required by `/plugin marketplace add volkan/duet`) + `commands/duet.md` make the repo installable as a Claude Code plugin providing `/duet` (or `/duet:duet` when Claude Code disambiguates namespaced commands); and `.agents/plugins/marketplace.json` + `plugins/duet/.codex-plugin/plugin.json` + `plugins/duet/skills/duet/SKILL.md` make the repo installable as a Codex plugin providing the `$duet` skill. Both plugin entry points shell out to the installed `duet` CLI; neither embeds duet.py. `docs/CLAUDE_CODE_PLUGIN.md` and `docs/CODEX_PLUGIN.md` are the dedicated install/use/troubleshooting guides. Keep both plugin manifest versions in lockstep with `pyproject.toml`'s.
