...
Packaging shims sit beside the file without changing its shape: `pyproject.toml` publishes it to PyPI as `duet-cli` (console script `duet = "duet:main"`, flat module `duet` via `py-modules`, `[yaml]` extra for PyYAML, zero runtime deps — bare `duet` on PyPI is Google's async library); the root `.claude-plugin/marketplace.json` (marketplace name `volkan-duet`, required by `/plugin marketplace add volkan/duet`) points at a narrowed Claude plugin root `plugins/duet-claude/` (`.claude-plugin/plugin.json` + `commands/duet.md`), making the repo installable as a Claude Code plugin providing `/duet` (or `/duet:duet` when Claude Code disambiguates namespaced commands); and `.agents/plugins/marketplace.json` + `plugins/duet/.codex-plugin/plugin.json` + `plugins/duet/skills/duet/SKILL.md` make the repo installable as a Codex plugin providing the `$duet` skill. Both plugin entry points shell out to the PATH-installed `duet` CLI; neither *invokes* a bundled `duet.py`. (The Claude marketplace `source` is `./plugins/duet-claude` — a narrow subtree of just the manifest + command — so the Claude plugin cache no longer contains `duet.py`, tests, docs, or workflows. The Claude and Codex plugins live in sibling roots, `plugins/duet-claude/` vs `plugins/duet/`, so neither leaks the other's components — e.g. a Codex `skills/duet/SKILL.md` under a shared root would surface as a duplicate Claude `duet` component.) `docs/CLAUDE_CODE_PLUGIN.md` and `docs/CODEX_PLUGIN.md` are the dedicated install/use/troubleshooting guides. Keep both plugin manifest versions in lockstep with `pyproject.toml`'s.
...
Six merge gates, in order of granularity. `.github/workflows/ci.yml` runs the runtime checks on every PR (unit + reasoning + smoke on Python 3.9/3.11/3.13), plus source/package metadata validation, Claude Code plugin validation, and complexity once. The source metadata validation covers both Claude and Codex plugin manifests. `make ci` runs the fast local subset that does not install external CLIs or build frontends. All jobs are advisory until marked **required** in branch protection; see `.github/BRANCH_PROTECTION.md` (admins can still force-merge). A separate `.github/workflows/release.yml` triggers `on: push: main` and detects a version bump (the `[project]` version changed in that push **and** no matching `vX.Y.Z` tag exists yet), then runs gate (`make ci`) → build (sdist+wheel) → OIDC PyPI publish (gated by the protected `pypi` environment approval) → auto-creates the `vX.Y.Z` tag + GitHub Release with generated notes, all in one run; it is **not** a required PR check and is not part of branch protection. The release runbook is `docs/RELEASING.md`. `.github/workflows/bump-version.yml` is a `workflow_dispatch` helper that bumps the three lockstep manifests (via `scripts/bump_release_version.py`, stdlib-only) and opens the `chore: release X.Y.Z` PR — merging that PR is the publish trigger.

1. `tests/test_duet.py` (run via `make unit-test` or `python3 -m unittest discover -s tests`) — pure-function unit tests, stdlib `unittest` only. Covers `_convergence_markers` / `convergence_proposed`, `_parse_codex_session_id`, `_parse_copilot_jsonl`, `_parse_gemini_session_id`, `parse_recap_headers`, `extract_files_heuristic`, the reasoning maps + `validate_reasoning` / `effective_reasoning`, `parse_partner`, `_markdown_fence`, `_humanize_age`, `derive_status_heuristic`, `_next_speaker_idx_from_state`, `_format_byte_size`, `normalize_verify_cmd`, and `_resolve_opt_path`. No subprocesses, no filesystem writes, no agent CLIs — runs in well under a second. Add a test in the same commit as any change to those helpers.
...
4. `scripts/check_distribution_metadata.py` (run via `make distribution-check`, and after builds via `python3 scripts/check_distribution_metadata.py --artifacts dist`) — validates `pyproject.toml`, Claude/Codex plugin manifests, marketplace entries, version lockstep, the forbidden employer-email regression, and built wheel/sdist metadata including absolute README links.
...
- **`Agent`** (dataclass) — `backend` (`claude`|`codex`|`gemini`|`copilot`), `role` (`planner`|`coder`|`reviewer`|custom), `session_id` (mutated across turns), `cwd_override` (set when this agent runs in a worktree), `reasoning_effort` (per-agent override of cfg-level reasoning), `extra_args`, `role_prompt`.
...
| new / changed pure helper covered by `tests/test_duet.py` (convergence detector, codex session-id parser, copilot JSONL parser, recap header parser, file-path heuristic, partner spec parser, markdown fence sizer, age formatter, status heuristic) | add a case to `tests/test_duet.py` describing the new behavior in the same commit |
...
| packaging / plugin metadata change (`pyproject.toml` name, version, console script, extras; `plugins/duet-claude/.claude-plugin/plugin.json` / root `.claude-plugin/marketplace.json`; `plugins/duet/.codex-plugin/plugin.json` / `.agents/plugins/marketplace.json`) | `README.md` install section (uvx/pipx/plugin snippets); `docs/CLAUDE_CODE_PLUGIN.md`, `docs/CODEX_PLUGIN.md`, and the concise `docs/USAGE.md` plugin sections if install steps change; the Makefile `build` target if the build tooling changes; keep plugin manifest versions matching `pyproject.toml` |
