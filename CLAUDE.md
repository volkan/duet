...
**duet** is a single-file Python harness (`duet.py`, ~3800 lines, stdlib-only, Python 3.9+) that runs two CLI agents — `claude` and `codex` by default, with `gemini`, `copilot`, and `opencode` available as additional backends and same-backend pairings supported — in alternating turns until both agents propose convergence in back-to-back turns with an LGTM rationale plus the sentinel (`<<<LGTM>>>` on its own line), hit `--turns`, time out, or get Ctrl-C'd. With `--on-turn-timeout continue` (the review recipe's default; plain runs default to `stop`) a single loop-turn timeout is not terminal: the timeout block — plus the worktree handoff/diff when the timed-out agent ran in a worktree — becomes that agent's reply and the partner takes the next turn, while two consecutive timeouts, a final-turn timeout, or a Ctrl-C during the timed-out turn still end the run with `reason=timeout` (`_timeout_may_continue` is the pre-call eligibility that also keeps an eligible timeout out of the persisted `finished_reason`; `_continue_after_timeout` re-checks the stop flag after the call so SIGINT beats continuation; seed extraction and forced turns are always terminal on timeout). A timed-out or errored turn does **not** mark the agent as seen — `seen_first_turn` flips only on a successful call, so a retry runs under first-turn rules and the same-cwd codex guards re-validate it. Each agent keeps its own conversation memory across turns: Claude via `--resume <session_id>` parsed from its JSON-wrapped output; Codex via `codex exec resume <uuid>` (UUID parsed out of Codex's stderr by `_parse_codex_session_id`) with a `codex exec resume --last` cwd-keyed fallback for builds that don't print one or for legacy continued runs; Gemini via JSON `session_id` from `gemini -p ... --output-format json` and `gemini --resume <session_id>`; Copilot via JSONL `sessionId` from `copilot -p ... --output-format json` and `copilot --resume=<sessionId>`; OpenCode via the top-level `sessionID` in the JSONL event stream from `opencode run --format json` and `opencode run -s <sessionID>` (the id is stable across turns, so resume is always id-keyed, never cwd-keyed).
...
Packaging shims sit beside the file without changing its shape:
`duet.__version__` is the canonical runtime version and `pyproject.toml` derives
`duet-cli` from it (console script `duet = "duet:main"`, flat module `duet`,
optional `[yaml]` extra, zero runtime dependencies). Both marketplaces point
...
Keep both native manifest versions in lockstep with `duet.__version__`.
[Installation and upgrades](docs/INSTALLATION.md) documents the shared and
native routes.
...
1. **The reasoning translation layer.** Six duet-abstraction levels (`REASONING_LEVELS = ["minimal","low","medium","high","xhigh","max"]`) → backend-specific maps (`CLAUDE_REASONING_MAP`, `CODEX_REASONING_MAP`, `GEMINI_REASONING_MAP`, `COPILOT_REASONING_MAP`, `OPENCODE_REASONING_MAP`) and prompt-prefix tables → the actual `--effort`/`-c model_reasoning_effort=`/`--variant` emission inside adapter calls. Codex passes every explicit level through unchanged, including `medium` and `max`; selected-model support is Codex's responsibility, so duet never silently substitutes another effort. An omitted reasoning value still uses the selected model's native default. Claude `minimal` maps to `low` (Claude has no `minimal`); Claude `xhigh` and `max` pass through. Copilot `minimal` maps to `none`; Copilot `xhigh` and `max` pass through. OpenCode uses an identity map (every level emits `run --variant <level>`): variants are provider-specific, but OpenCode silently ignores one a model doesn't define, so passing the level through is forward-compatible and never errors. Gemini has no documented effort flag, so it emits no backend effort argument; high/xhigh/max only tack a "think hard"/"think very hard"/"ultrathink" prefix onto the prompt (Claude/Gemini/Copilot/OpenCode all reuse `CLAUDE_REASONING_PROMPT_PREFIX`). `scripts/check_reasoning_levels.py` monkey-patches `_run` and asserts each level produces the right cmd; rerun it after touching any of these constants.

   **Reasoning-scaled default timeout.** When no explicit per-turn timeout is given, `derive_default_per_turn_timeout` derives the default from the *per-agent effective* levels (`effective_reasoning(agent, resolved_global)` — an override replaces the global, so a global `max` that every agent overrides to `low` derives 900, not 1800): 900 base, 1200 at `high`, 1800 at `xhigh`/`max`, and emits the one-line stderr note itself. `resolve_default_per_turn_timeout` remains the pure resolver used by its unit tests. Precedence is path-specific and pre-existing: plain CLI — `--timeout` beats the derived default; `--config` — file `per_turn_timeout` beats `--timeout` beats derived (file-wins matches sentinel/sandbox/permission_mode, see `_yaml_per_turn_timeout`); `--continue` — `--timeout` beats state beats derived (`_continue_default_timeout`, which only fires for legacy states missing the key). `codex_fast` deliberately never affects derivation: it only lowers one role's effort, and the single per-run budget must still cover the slowest agent.
...
2. **The launch/status/stop control plane.** `_prepare_run` must write initial state (`kickoff_pending` when applicable) before executing `task_from_cmd`, then atomically publish `--run-info-file` without overwriting. Raw task commands never enter state/run-info. `state.json["duet_pid"]`, `state.json["duet_process_start"]`, transient `turn-*.pid`, stderr logs, and the saved `phase` feed control operations. JSON status is schema 1 and curated: never add task/kickoff/verify commands, credentials, `extra_args`, or raw errors/history. Exit codes are 0 terminal (any reason), 1 running, 2 stuck/crashed, 3 status error. Every state payload must still come from `_build_run_state`; phase writes cover kickoff pending/running, turn running, between turns, awaiting force, and finished. `--stop` must validate the exact supervisor PID and saved process-start identity twice before signaling only that PID. It must fail closed for terminal, invalid, stale, PID 1, reused, foreign, unprovable, and ambiguous bare-ID targets. A stop at `awaiting_force` can remain blocked in TTY input; after input returns, `ask_force` must preserve `force_stop`.

   The schema exposes the sanitized top-level `per_turn_timeout`; an active turn also exposes `budget_seconds` and zero-clamped `remaining_seconds`. `last_completed_turn.finished_reason` exposes that turn's sanitized outcome, while top-level `last_timeout` retains the most recent timeout even after a later partner turn completes, so absorbed timeouts remain observable without exposing raw history. `_positive_int_or_none` must coerce the untrusted saved value before either publication or arithmetic, so strings, booleans, zero, and negative values become `null` without breaking the status exit-code contract.

   Status artifact paths are absolute in new state files; the reader also re-anchors legacy relative paths from the resolved run directory, not `state["cwd"]` (an explicit invocation-relative `--runs-dir` may live elsewhere). Duet PID recognition must handle both `python .../duet.py` source launches and `python .../bin/duet` wheel console scripts. When a sandbox permits signal-0 liveness but blocks command-line inspection, `_is_duet_process` accepts the live PID except PID 1; otherwise installed/sandboxed runs are falsely reported dead between turns or at `force>`.
...
`subprocess.Popen` with `start_new_session=True` so we own a process group (clean SIGTERM/SIGKILL via `os.killpg`). Two reader threads drain stdout/stderr line-by-line; stderr is mirrored to the user's terminal (with `LIVE_PREFIX = "  │ "`) **and** tee'd to a per-turn `*.stderr.log` file. `pid_file_path` is written atomically (temp+rename) on Popen and removed in `finally`. `_terminate_active_processes(SIGKILL)` walks `_ACTIVE_PROCS` (a thread-locked set) on second-Ctrl-C. A run-scoped immediate stop sends SIGTERM only to the validated supervisor PID. Its handler uses the same tracked per-child process-group cleanup and records `force_stop`.

Failed-turn transcript blocks must go through `format_agent_error_for_transcript`: it keeps a bounded head/tail excerpt (`AGENT_ERROR_TRANSCRIPT_MAX_CHARS`) and points at the per-turn stderr log, which remains the complete forensic record. Do not inline an unbounded `str(exc)` into `transcript.md`. The same rule applies to every **persisted** error: `history[].error` (loop, forced, and seed-extract turns) goes through `_bounded_agent_error_excerpt`, and the first-line summaries in `_launch_failure_state`/kickoff go through `_error_summary` (`ERROR_SUMMARY_MAX_CHARS`) — a raw xhigh-codex stderr dump once ballooned `state.json` to 437KB. Adapter exit errors are built by `_agent_exit_error`, which appends a bounded stdout tail when stderr is blank (`claude -p --resume` has failed with exit 1 and a completely silent stderr); `setup_worktree` failures always carry rc plus bounded tails of both streams, because git's stderr can hold only the "Preparing worktree…" progress line.
...
- **Central metrics use a distinct store.** Curated snapshots belong under `~/.duet/metrics/runs/<UUID>.json`, never in the legacy `~/.duet/runs` index or its symlink tree. Snapshot publication is atomic and best effort: a home-directory failure warns but must not fail the agent run. Keep raw transcripts, state, prompts, paths, commands, errors, credentials, and session IDs out of central snapshots; missing provider metadata stays unknown rather than inferred.
