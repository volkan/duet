...
Packaging shims sit beside the file without changing its shape: `pyproject.toml` publishes it to PyPI as `duet-cli` (console script `duet = "duet:main"`, flat module `duet` via `py-modules`, `[yaml]` extra for PyYAML, zero runtime deps — bare `duet` on PyPI is Google's async library); `.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json` (marketplace name `volkan-duet`, required by `/plugin marketplace add volkan/duet`) + `commands/duet.md` make the repo installable as a Claude Code plugin providing `/duet` (or `/duet:duet` when Claude Code disambiguates namespaced commands); and `.agents/plugins/marketplace.json` + `plugins/duet/.codex-plugin/plugin.json` + `plugins/duet/skills/duet/SKILL.md` make the repo installable as a Codex plugin providing the `$duet` skill. Both plugin entry points shell out to the PATH-installed `duet` CLI; neither *invokes* a bundled `duet.py`. (The Claude Code plugin's `source: "./"` does copy `duet.py` into the plugin cache as a side effect of bundling the repo root — `~/.claude/plugins/cache/volkan-duet/duet/<version>/duet.py` — but the `/duet` command still resolves `duet` from PATH, not that copy. The Codex plugin's `source.path: "./plugins/duet"` is a subtree that excludes `duet.py`, so the Codex cache never contains it.) `docs/CLAUDE_CODE_PLUGIN.md` and `docs/CODEX_PLUGIN.md` are the dedicated install/use/troubleshooting guides. Keep both plugin manifest versions in lockstep with `pyproject.toml`'s.
...
| change to the `/duet` slash-command recipe | `commands/duet.md` (the plugin command) and `docs/CLAUDE_CODE_PLUGIN.md` (install/use/troubleshooting); keep `docs/USAGE.md` as the concise reference link/summary; `.claude-plugin/plugin.json` description if the behavior summary shifts (`marketplace.json` deliberately carries only a short listing blurb, so it rarely needs the same edit) |
...
| packaging / plugin metadata change (`pyproject.toml` name, version, console script, extras; `.claude-plugin/plugin.json` / `marketplace.json`; `plugins/duet/.codex-plugin/plugin.json` / `.agents/plugins/marketplace.json`) | `README.md` install section (uvx/pipx/plugin snippets); `docs/CLAUDE_CODE_PLUGIN.md`, `docs/CODEX_PLUGIN.md`, and the concise `docs/USAGE.md` plugin sections if install steps change; the Makefile `build` target if the build tooling changes; keep plugin manifest versions matching `pyproject.toml` |
