...
| packaging / plugin metadata change (`pyproject.toml` name, version, console script, extras; `.claude-plugin/plugin.json` / `marketplace.json`) | `README.md` install section (uvx/pipx/plugin snippets); `docs/CLAUDE_CODE_PLUGIN.md` and the concise `docs/USAGE.md` `/duet` section if install steps change; the Makefile `build` target if the build tooling changes; keep `plugin.json` version matching `pyproject.toml` |
